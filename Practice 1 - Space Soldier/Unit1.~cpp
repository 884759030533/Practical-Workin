...
void player_bullet(int bulletID, int bulletX, int bulletY)
{
    if (b_counter<=8)
    {
        if (grid_pos[bulletX][bulletY]!=0) for (int i=0; i<6; i++)
        { grid_pos[bulletX][bulletY+i] = 0; return; }

        for (int i=0; i<10; i++)
        {
            if ((grid_pos[bulletX][bulletY+i]==1)||(grid_pos[bulletX][bulletY+i]==2)||(grid_pos[bulletX][bulletY+i]==3))
            {   grid_pos[bulletX][bulletY+i-1] = 20; return; }
            else if ((i>=6)/*||(grid_pos[bulletX][bulletY+i]==20)*/) grid_pos[bulletX][bulletY+i] = 0;
            else grid_pos[bulletX][bulletY+i] = bulletID+i;
        }
        if (bulletY<0) { for (int i=0; i<6; i++)
        { grid_pos[bulletX][bulletY+i] = 0; } }
    }
    else main_form->t_bullet->Enabled = false;
...
void __fastcall Tmain_form::main_gridKeyPress(TObject *Sender, char &Key)
{
    ...
    switch (Key)
    {
        ...
        case 32: { if (b_counter>=8)
            {
                bulletXY[0][0] = PlPosX+2;
                bulletXY[1][0] = PlPosY-1;
                b_counter = 0;
                t_bullet->Enabled = true; break;}
             else break; };
...
{
    if (b_counter<8) { b_counter++; debug_form->Label4->Caption = IntToStr(b_counter); }
    player_bullet(14, bulletXY[0][0], bulletXY[1][0]);
...
void __fastcall Tmain_form::T_tick_screenTimer(TObject *Sender)
{
    main_grid->Invalidate();
    debug_form->Label3->Caption = IntToStr(debug_tick_counter%10)+" ticks in second";
    debug_tick_counter++;
    //grid_pos[0][0] = 0;
    //draw_ship(1, PlPosX, PlPosY);
}
